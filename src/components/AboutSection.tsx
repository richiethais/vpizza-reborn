...
const AboutSection = () => {
  ...
  const { ref: imageRef, isVisible: imageVisible } = useScrollAnimation({ rootMargin: "0px 0px -80px 0px" });
  ...
  return (
    <section id="story" className="bg-background py-8 md:py-16 px-4 md:px-8">
      <div className="max-w-5xl mx-auto flex flex-col md:flex-row gap-6 md:gap-12 items-start">
        ...
        <div
          ref={imageRef}
          className={`flex-shrink-0 w-full md:w-auto transition-all duration-700 ease-out delay-200 ${
            imageVisible ? "opacity-100 translate-x-0 scale-100" : "opacity-0 translate-x-8 scale-95"
          }`}
        >
          <img src={sisig} alt="Sisig fried rice — fresh ingredients" className="w-full md:w-[400px] h-[220px] md:h-[350px] object-cover rounded" />
        </div>
      </div>
    </section>
  );
